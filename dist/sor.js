...
exports.MAX_UINT = ethers_1.utils.bigNumberify(ethers_1.ethers.constants.MaxUint256);
const minAmountOut = 0;
const maxAmountIn = exports.MAX_UINT;
const maxPrice = exports.MAX_UINT;
exports.smartOrderRouterMultiHop = (pools, paths, swapType, totalSwapAmount, maxPools, costReturnToken) => {
    paths.forEach(b => {
        b.spotPrice = helpers_1.getSpotPricePath(pools, b);
        b.slippage = helpers_1.getSlippageLinearizedSpotPriceAfterSwapPath(pools, b, swapType);
    ...
    let pricesOfInterest = getPricesOfInterest(sortedPaths, swapType).sort((a, b) => {
    ...
    for (let b = 1; b <= bmin; b++) {
        totalReturn = 0;
        let price, priceAfter, priceBefore, swapAmountsPriceBefore, swapAmountsPriceAfter;
        ...
        if (totalNumberOfPools <= maxPools) {
            if (swapType === 'swapExactIn') {
                ...
                improvementCondition =
                    totalReturnConsideringFees.isGreaterThan(bestTotalReturnConsideringFees) ||
                        bestTotalReturnConsideringFees.isEqualTo(new bignumber_1.BigNumber(0));
            }
            else {
                ...
                improvementCondition =
                    totalReturnConsideringFees.isLessThan(bestTotalReturnConsideringFees) ||
                        bestTotalReturnConsideringFees.isEqualTo(new bignumber_1.BigNumber(0));
            }
        }
...
function getPricesOfInterest(sortedPaths, swapType) {
    ...
    sortedPaths.forEach((b, i) => {
        ...
        pi = {};
        pi.price = b.spotPrice.plus(bmath_1.bmul(b.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
        pi.maxAmount = b.id;
        pricesOfInterest.push(pi);
        for (let k = 0; k < i; k++) {
            let prevPath = sortedPaths[k];
            if (bmath_1.bmul(b.slippage, b.spotPrice).isLessThan(bmath_1.bmul(prevPath.slippage, prevPath.spotPrice))) {
                let amountCross = bmath_1.bdiv(b.spotPrice.minus(prevPath.spotPrice), bmath_1.bmul(prevPath.slippage, prevPath.spotPrice).minus(bmath_1.bmul(b.slippage, b.spotPrice)));
                if (amountCross.isLessThan(b.limitAmount) &&
                    amountCross.isLessThan(prevPath.limitAmount)) {
                    let epiA = {};
                    epiA.price = b.spotPrice.plus(bmath_1.bmul(amountCross, bmath_1.bmul(b.slippage, b.spotPrice)));
                ...
                if (prevPath.limitAmount.isLessThan(b.limitAmount) &&
                    ...
                    let epiB = {};
                    epiB.price = b.spotPrice.plus(bmath_1.bmul(prevPath.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
                    epiB.swap = [prevPath.id, b.id];
                    pricesOfInterest.push(epiB);
                }
                if (b.limitAmount.isLessThan(prevPath.limitAmount) &&
                    amountCross.isLessThan(b.limitAmount)) {
                    let epiC = {};
                    epiC.price = prevPath.spotPrice.plus(bmath_1.bmul(b.limitAmount, bmath_1.bmul(prevPath.slippage, prevPath.spotPrice)));
                ...
                if (prevPath.limitAmount.isLessThan(b.limitAmount)) {
                    let epiD = {};
                    epiD.price = b.spotPrice.plus(bmath_1.bmul(prevPath.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
...
function getSwapAmountsForPriceOfInterest(paths, pathIds, poi) {
    ...
    pathIds.forEach((bid, i) => {
        ...
        let inputAmount = bmath_1.bdiv(poi.minus(path.spotPrice), bmath_1.bmul(path.slippage, path.spotPrice));
