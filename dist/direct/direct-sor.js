...
exports.smartOrderRouter = (balancers, swapType, targetInputAmount, maxBalancers, costOutputToken) => {
    balancers.forEach(b => {
        b.spotPrice = helpers_1.getSpotPrice(b);
        b.slippage = helpers_1.getSlippageLinearizedSpotPriceAfterSwap(b, swapType);
    ...
    let epsOfInterest = getEpsOfInterest(sortedBalancers, swapType).sort((a, b) => {
    ...
    for (let b = 1; b <= bmin; b++) {
        ...
        if (swapType === 'swapExactIn') {
            totalOutput = totalOutput.minus(bmath_1.bmul(new bignumber_1.BigNumber(balancerIds.length).times(bmath_1.BONE), costOutputToken));
            improvementCondition =
                totalOutput.isGreaterThan(bestTotalOutput) ||
                    bestTotalOutput.isEqualTo(new bignumber_1.BigNumber(0));
        }
        else {
            totalOutput = totalOutput.plus(bmath_1.bmul(new bignumber_1.BigNumber(balancerIds.length).times(bmath_1.BONE), costOutputToken));
            improvementCondition =
...
exports.smartOrderRouterEpsOfInterest = (balancers, swapType, targetInputAmount, maxBalancers, costOutputToken, epsOfInterest) => {
    let bestTotalOutput = new bignumber_1.BigNumber(0);
    let highestEpNotEnough = true;
    let balancerIds, totalOutput;
    let bestInputAmounts, bestBalancerIds, inputAmounts;
...
function getEpsOfInterest(sortedBalancers, swapType) {
    ...
    sortedBalancers.forEach((b, i) => {
        ...
        epi = {};
        epi.price = b.spotPrice.plus(bmath_1.bmul(b.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
        epi.maxAmount = b.id;
        epsOfInterest.push(epi);
        for (let k = 0; k < i; k++) {
            let prevBal = sortedBalancers[k];
            if (bmath_1.bmul(b.slippage, b.spotPrice).isLessThan(bmath_1.bmul(prevBal.slippage, prevBal.spotPrice))) {
                let amountCross = bmath_1.bdiv(b.spotPrice.minus(prevBal.spotPrice), bmath_1.bmul(prevBal.slippage, prevBal.spotPrice).minus(bmath_1.bmul(b.slippage, b.spotPrice)));
                if (amountCross.isLessThan(b.limitAmount) &&
                    amountCross.isLessThan(prevBal.limitAmount)) {
                    let epi1 = {};
                    epi1.price = b.spotPrice.plus(bmath_1.bmul(amountCross, bmath_1.bmul(b.slippage, b.spotPrice)));
                ...
                if (prevBal.limitAmount.isLessThan(b.limitAmount) &&
                    ...
                    let epi2 = {};
                    epi2.price = b.spotPrice.plus(bmath_1.bmul(prevBal.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
                    epi2.swap = [prevBal.id, b.id];
                    epsOfInterest.push(epi2);
                }
                if (b.limitAmount.isLessThan(prevBal.limitAmount) &&
                    amountCross.isLessThan(b.limitAmount)) {
                    let epi3 = {};
                    epi3.price = prevBal.spotPrice.plus(bmath_1.bmul(b.limitAmount, bmath_1.bmul(prevBal.slippage, prevBal.spotPrice)));
                ...
                if (prevBal.limitAmount.isLessThan(b.limitAmount)) {
                    let epi4 = {};
                    epi4.price = b.spotPrice.plus(bmath_1.bmul(prevBal.limitAmount, bmath_1.bmul(b.slippage, b.spotPrice)));
...
exports.calcTotalOutput = (swaps, poolData) => {
    try {
        ...
        swaps.forEach(swap => {
            ...
            const preview = bmath_1.calcOutGivenIn(pool.balanceIn, pool.weightIn, pool.balanceOut, pool.weightOut, bmath_1.bnum(swapAmount), pool.swapFee);
...
exports.calcTotalInput = (swaps, poolData) => {
    try {
        ...
        swaps.forEach(swap => {
            ...
            const preview = bmath_1.calcInGivenOut(pool.balanceIn, pool.weightIn, pool.balanceOut, pool.weightOut, bmath_1.bnum(swapAmount), pool.swapFee);
...
function getInputAmountsForEp(balancers, bids, ep) {
    ...
    bids.forEach((bid, i) => {
        ...
        let inputAmount = bmath_1.bdiv(ep.minus(balancer.spotPrice), bmath_1.bmul(balancer.slippage, balancer.spotPrice));
...
function getLinearizedTotalOutput(balancers, swapType, balancerIds, inputAmounts) {
    let balancer;
    ...
    balancerIds.forEach((b, i) => {
        ...
        totalOutput = totalOutput.plus(helpers_1.getLinearizedOutputAmountSwap(balancer, swapType, inputAmounts[i]));
    });
    return totalOutput;
}
function getExactInputAmounts(inputAmountsEpBefore, inputAmountsEpAfter, targetTotalInput) {
    let deltaInputAmounts = [];
    if (inputAmountsEpAfter[inputAmountsEpAfter.length - 1].isEqualTo(new bignumber_1.BigNumber(0)))
    ...
    deltaInputAmounts.forEach((a, i) => {
        let div = bmath_1.bdiv(targetTotalInput.minus(totalInputBefore), deltaTotalInput);
